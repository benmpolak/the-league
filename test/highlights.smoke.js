/* Match highlights links.
 *
 * Marc, 22 Aug: "the highlights are just giving a generic link to youtube, not
 * the youtube links to the specific match." Four of our club names are FPL
 * abbreviations that appear in no video title anywhere ("Man Utd", "Spurs",
 * "Nott'm Forest", "Man City"), so searches carrying them found nothing and
 * read as a generic page.
 *
 * Main's builder (fxYtHref) prefers the curated per-fixture video id and falls
 * back to a search deep-link; the abbreviation fix applies to that fallback.
 * The failure is silent by nature — a bad query still renders a perfectly
 * valid link — so this pins the query text rather than the markup, and guards
 * against a future feed introducing another abbreviation nobody notices for a
 * month.
 */
'use strict';
...
(async () => {
  ...
  const report = await page.evaluate(() => {
    ...
    // a real fixture through the fallback, end to end
    const f = state.fixtures.find(x => x.hs != null && x.as != null) || state.fixtures[0];
    delete (state.highlights || {})[String(f.id)]; // force the search fallback
    const url = fxYtHref(f);
    const q = decodeURIComponent(url.split('search_query=')[1] || '');
    ok('without a curated id the link is a search deep-link', url.startsWith('https://www.youtube.com/results?search_query='), url.split('?')[0]);
    ok('the query names both clubs as Sky titles them', q.includes(skyName(f.home)) && q.includes(skyName(f.away)), q);
    ok('the query carries the score, which is what pins the exact video',
      f.hs == null || q.includes(`${f.hs}-${f.as}`), q);
    ok('the query is encoded, so a club with a space survives', !/ /.test(url.split('search_query=')[1] || ''));

    // the curated map still wins when it has this fixture
    state.highlights = { ...(state.highlights || {}), [String(f.id)]: 'dQw4w9WgXcQ' };
    ok('a curated video id beats the search', fxYtHref(f) === 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', fxYtHref(f));
    delete state.highlights[String(f.id)];
    ...
    ok('and each is exactly what the builder produces',
      links.length > 0 && shown.every(x => links.includes(fxYtHref(x))),
    ...
    const queries = links.map(h => decodeURIComponent(h.split('search_query=')[1] || ''));
    const bad = queries.filter(q2 => /\bMan Utd\b|\bMan City\b|\bSpurs\b|Nott'm/.test(q2));
    ok('no link still asks for an FPL abbreviation', bad.length === 0, bad[0] || queries[0] || 'no search links');
    return out.join('\n');
  });
